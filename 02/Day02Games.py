inputString = '''Game 1: 1 green, 1 blue, 1 red; 1 green, 8 red, 7 blue; 6 blue, 10 red; 4 red, 9 blue, 2 green; 1 green, 3 blue; 4 red, 1 green, 10 blue
...
def parseUntilDelimiter(inputString, delimiter):
    index = inputString.find(delimiter)
    if index != -1:
        beforeDelim = inputString[:index]
        rest = inputString[index + 1:]
        return beforeDelim, rest
    else:
        return inputString, None

def checkSet(input):
    while True:
        cube, input = parseUntilDelimiter(input, ',')
        count, colour = cube.split()
        if int(count) > cubeLimit[colour]:
            return False
        if input is None:
           return True
...
def checkGame(line):
    while True:
        set, line = parseUntilDelimiter(line, ';')
        print(f'{set}') if debug else None
