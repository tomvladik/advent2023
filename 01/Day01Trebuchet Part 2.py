inputString = '''7jlncfksix7rjgrpglmn9
...
windowMaxSize = max(len(word) for word in word2number.keys())
windowMinSize = min(len(word) for word in word2number.keys())

def findNumberWord(inputString, startIndex):
    for windowSize in range(windowMinSize, windowMaxSize+1):
        window = inputString[startIndex:startIndex + windowSize]
        possiblyNumber = word2number.get(window, None)
        if possiblyNumber is not None:
...
for line in lines:
    ...
    while i < lineLength:
            ...
            if char.isdigit():
                ...
                lineValue += 10 * num
                break
            possiblyNumber = findNumberWord(line, i)
            if possiblyNumber is not None:
               lineValue += 10 * possiblyNumber
               break
            i += 1     
    
    #locate right most number
    index = len(line) - 1  # Set the index to the last character
    while index >= 0:
        char = line[index]
        if char.isdigit():
            num = int(char)
            lineValue += num
            break
        possiblyNumber = findNumberWord(line, index)
        if possiblyNumber is not None:
            lineValue += possiblyNumber
            break
...
print(f'**** sum of all of the calibration values: {theSum} ****')